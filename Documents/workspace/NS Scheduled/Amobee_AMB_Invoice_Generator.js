...
function invGenerator(type) {
	...
	if (deploy && deploy == 'customdeploy1') {
		var searchResults = nlapiSearchRecord('transaction', 'customsearch_amb_inv_creator_num1', null, null);
	...
	if (searchResults && searchResults != '') {
		...
		for (var i=0; i< theLength; i++) {
			...
			var disc = {};
			var targ = searchResults[i].getValue('custcol_amb_targeting', null, 'group');
			...
			if (campId && campId != '') {
				...
				if (invId && invId != '') {
					...
					for (var j=1; j<=lineCount; j++) {
						...
						if (recInv.getLineItemValue('item', 'custcol_amb_targeting', j) == searchResults[i].getValue('custcol_amb_targeting', null, 'group')) {
							...
							var newQuan = parseFloat(quan) + parseFloat(searchResults[i].getValue('quantity',null,'sum'));
						...
						/*	var ioLineItemName = searchResults[i].getValue('custcol_sf_io_line_item_name', null, 'group');
							...
							rate = round(rate, 5);
							var customerDiscount = searchResults[i].getValue('custcol_amb_so_cust_discount', null, 'max');
							var managementFee = searchResults[i].getValue('custcol_amb_mgmt_fee', null, 'max');
							var managementFeeOn = searchResults[i].getValue('custbody_amb_calc_mgmt_fee', null, 'group');
							
							
							
							var fulfillmentGrossRate = calculateGrossRate(rate, customerDiscount, managementFee, managementFeeOn)
							
							recInv.setCurrentLineItemValue('item', 'quantity',  newQuan);
							recInv.setCurrentLineItemValue('item','custcol_amb_inv_ful_gross_rt', fulfillmentGrossRate) ;
							recInv.setCurrentLineItemValue('item', 'custcol_unit_price', rate );
							recInv.setCurrentLineItemValue('item', 'rate',rate);
							
							var fulfillmentGrossAmount = fulfillmentGrossRate;
							if(fulfillmentGrossRate && fulfillmentGrossRate != '')
							{
								fulfillmentGrossAmount = parseFloat(fulfillmentGrossRate) * parseFloat(newQuan);
							}
							
							
							//var grossAmount = discounter(newAmount, disc);
							recInv.setCurrentLineItemValue('item', 'custcol_amb_inv_ful_gross_amt', fulfillmentGrossAmount);
					...
					if (lineFound == false) {
						...
						grossItemRate = round(grossItemRate,6);
						//Util.console.log(grossItemRate, 'grossItemRate');
						var rate = parseFloat(searchResults[i].getValue('custcol_amb_so_net_spend', null, 'sum'))/parseFloat(searchResults[i].getValue('quantity',null,'sum'));
						rate = rate.toFixed(8);
						...
						var managementFee = searchResults[i].getValue('custcol_amb_mgmt_fee', null, 'max');
						var managementFeeOn = searchResults[i].getValue('custbody_amb_calc_mgmt_fee', null, 'group');
						var newQuan = searchResults[i].getValue('quantity',null,'sum')
						...
						recInv.setCurrentLineItemValue('item', 'custcol_amb_targeting', searchResults[i].getValue('custcol_amb_targeting', null, 'group'));
						recInv.setCurrentLineItemValue('item', 'custcol_amb_so_cust_discount', customerDiscount);
						recInv.setCurrentLineItemValue('item', 'custcol_amb_mgmt_fee', managementFee);
					...
					recInv.setFieldValue('custbody_inv_appr_status', 2);
					...
					for (var j=lineCount; j>=1; j--) {
						...
						if (recInv.getLineItemValue('item', 'custcol_amb_targeting', j) == searchResults[i].getValue('custcol_amb_targeting', null, 'group')) {
							...
							var newQuan = searchResults[i].getValue('quantity',null,'sum');
							var fulfillmentGrossAmount = fulfillmentGrossRate;
							Util.console.log(fulfillmentGrossAmount, 'fulfillGrossAmount before');
							if(fulfillmentGrossRate && fulfillmentGrossRate != '')
							{
								fulfillmentGrossAmount = parseFloat(fulfillmentGrossRate) * parseFloat(newQuan);
							}
							
							Util.console.log(fulfillmentGrossAmount, 'fulfillGrossAmount after');
				...
				if (invId && invId != '') {
					updateFulfillRecs(targ, invId, trandate);
				}
...
function calculateGrossRate(lineRate, customerDiscount, managementFee, managementFeeOn)
{
	var fulfillmentGrossRate = lineRate;
	if(!customerDiscount || customerDiscount == '')
	{
		customerDiscount = -1;
	}
	else
	{
		customerDiscount = 1 - (parseFloat(customerDiscount) / 100);
	}
	if(!managementFee || managementFee == '')
	{
		managementFee = -1;
	}
	else
	...
	if(managementFeeOn == '2') // NET
	{
		if(customerDiscount != -1 && managementFee != -1)
		{
			fulfillmentGrossRate = parseFloat(lineRate) / (parseFloat(customerDiscount) * (1 + parseFloat(managementFee)));
		}
		else
		{
			if(customerDiscount != -1)
			{
				fulfillmentGrossRate = parseFloat(lineRate) / (parseFloat(customerDiscount));
			}
			else
			{
				if(managementFee != -1)
				{
					fulfillmentGrossRate = parseFloat(lineRate) / (1 + parseFloat(managementFee));
				}										
			}
		}
	}
	...
	{	
		if(customerDiscount != -1 && managementFee != -1)
		{
			fulfillmentGrossRate = parseFloat(lineRate) / (parseFloat(customerDiscount) + parseFloat(managementFee));
		}
		else
		{
			if(customerDiscount != -1)
	...
	return fulfillmentGrossRate;
}

function updateFulfillRecs(targ, invId, trandate) {
	
	var startDate = getMonthDate('start', trandate);
	var endDate = getMonthDate('end', trandate);
	
	var filters = [
	    new nlobjSearchFilter('custcol_amb_targeting',null, 'anyof', targ),
